...
type OrderRecord = {
  ...
  events: { organizer_id: string; title: string } | Array<{ organizer_id: string; title: string }> | null
  order_items: Array<{ id: string; ticket_type_id: string; quantity: number; unit_price_kobo: number | string; ticket_types: { name: string } | Array<{ name: string }> | null }>
}
...
async function sendTicketEmail(order: OrderRecord, tickets: Array<{ ticketCode: string; qrDataUrl: string }>) {
  ...
  try {
    ...
    const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [order.customer_email], subject: `Your Elora ticket for ${event?.title ?? 'your event'}`, html: `<div style="font-family:Arial,sans-serif;color:#070a10"><h1>Your ticket is ready.</h1><p>Hi ${order.customer_name ?? 'there'}, your payment was verified. Show the QR code below at the venue.</p>${ticketMarkup}<p>Keep this email available on your phone.</p></div>` }) })
    return response.ok
  } catch (error) {
...
async function sendOrganizerEmail(order: OrderRecord) {
  ...
  const event = firstRelation(order.events)
  const { data } = await admin.auth.admin.getUserById(event?.organizer_id ?? '')
  const organizerEmail = data.user?.email
  if (!organizerEmail) return false
  try {
    const event = firstRelation(order.events)
    const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [organizerEmail], subject: `New ticket sale for ${event?.title ?? 'your event'}`, html: `<div style="font-family:Arial,sans-serif;color:#070a10"><h1>New ticket sale.</h1><p>A customer bought tickets for <strong>${event?.title ?? 'your event'}</strong>.</p><p>Customer: ${order.customer_name ?? 'Not provided'}<br />Email: ${order.customer_email ?? 'Not provided'}<br />Order total: N${(Number(order.total_kobo) / 100).toLocaleString('en-NG')}</p><p>Open your Elora organizer dashboard to review the sale.</p></div>` }) })
    return response.ok
  } catch (error) {
