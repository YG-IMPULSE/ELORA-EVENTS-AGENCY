...
export default function OrganizersPage() {
  ...
  const [isOrganizer, setIsOrganizer] = useAuthState(false)

  useEffect(() => { createClient().auth.getUser().then(({ data }) => setIsOrganizer(data.user?.user_metadata?.role === 'organizer')) }, [setIsOrganizer])
  ...
  return (
    <main className="organizer-landing">
      ...
      <section className="organizer-hero"><div><p className="eyebrow">THE EVENT COMMERCE PLATFORM</p><h1>Turn your event<br /><strong>into a system.</strong></h1><p>Elora helps organizers sell tickets, manage entry and keep the customer journey clear from first click to final scan.</p><div className="organizer-hero-actions"><a className="button button--dark" href={isOrganizer ? '/organizers/dashboard' : '/organizers/sign-in'}>{isOrganizer ? 'Open organizer dashboard' : 'Sign in to create an event'} <span>→</span></a><span>Built for serious event teams.</span></div></div><div className="organizer-hero-panel"><span>ELORA / ORGANIZER CONTROL</span><strong>ONE EVENT.<br />EVERYTHING<br />CONNECTED.</strong><div><b>Tickets</b><b>Check-in</b><b>Records</b></div></div></section>
